...
class SaleExchangeStore
{
    public SaleExchangeNewAction $new_exchange;

    public function __construct(SaleExchangeNewAction $new_exchange)
    {
        $this->new_exchange = $new_exchange;
    }
    ...
    public function execute($request)
    {
        ...
        $return_calculation = $request->get('return_calculation');
        $return_products = $request->get('return_products');
        $sale = Sale::findOrFail($sale_id);

        $this->new_exchange->execute($request, $sale);

        $final_total = ($sale->final_total + $request->payable_amount) - $return_calculation['return_total'];
        $vat_amount = $sale->vat_amount - $return_calculation['vat_total'];
        $discount_amount = $sale->discount_amount - $return_calculation['discount_total'];
...
//        $sale->net_total = $net_total <= 0 ? 0 : $net_total;
        ...
        $sale->exchange_total += $request->sale_net_total;
        $sale->flat_discount = $invoice_calculation['flat_discount'];
        $sale->save();

        $sale_return = new Sale_return();
        $sale_return->return_type = Sale_return::RETURN_TYPE['exchange_return'];
        ...
        foreach ($return_products as $value) {
            ...
            $product_data = [
                'return_type' => SaleReturnDetail::RETURN_TYPE['exchange_return'],
            ...
            foreach ($purchases as $purchaseId => $purchase) {
                ...
                $sale_return_history[] = [
                    'return_type' => SaleReturnPurchaseHistory::RETURN_TYPE['exchange_return'],
