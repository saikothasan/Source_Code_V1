...
{
    public function handle( $request, Product $product)
    {
        $data = $request->validated();
        $product->fill($data)->save();

        ProductOption::query()->where('product_id',$data['id'])->delete();
        ProductVariantSkuBarcode::query()->where('product_id',$data['id'])->delete();
        ProductVariantValues::query()->where('product_id',$data['id'])->delete();
