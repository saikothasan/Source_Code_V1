                ...
                <div class="row">
                    <div class="col-md-12 row">
                        <div class="col-md-12 text-center">
                            <h3>{{translate('Sections')}} {{translate('View')}}</h3>
                        </div>
                    </div>
                </div>
                        ...
                        @if (isset($section))
                            <form method="POST" action="{{ route('sections.update', $section->id) }}">
                                ...
                                <div class="form-group custom-btn">
                                    <input type="text" autocomplete="off" class="form-control corner" name="name"
                                        value="{{ $section->name }}">
                                    <input type="hidden" class="form-control corner" value="web" name="guard_name">
                                    @error('name')
                            ...
                            <form method="POST" action="{{ route('sections.store') }}">
                                ...
                                <div class="form-group custom-btn">
                                    ...
                                    @error('name')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                    <button class="btn custom-add font btn-success" type="submit">{{translate('Add')}}</button>
                                </div>
                            </form>
                        @endif
                    </div>
