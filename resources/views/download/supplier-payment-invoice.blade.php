                    ...
                    <div class="col-md-4">
                        <div class="content-wrapper" id="print" style="min-height: 282px;">
                            <div class="ticket invoice invoice-font invoice-border" style="margin: auto;">
                                ...
                                <h3 class="centered">
                                    <strong>Supplier</strong> Paynemt
                                </h3>
                                @php
                                    $invoices = collect(json_decode($purchasePayment->purchase_invoice));
                                @endphp
                                <p class="text-left">Invoice : <strong> {!! $invoices->implode('<br>') !!} </strong></p>
                                <p class="text-left"> Total Amount : <strong>
                                        {{ $purchasePayment->total_pay + $purchasePayment->total_due + $purchasePayment->total_advance }}
                                    </strong> </p>
                                <p class="text-left">Advance : <strong>{{ $purchasePayment->total_advance }} </strong>
                                    BDT </p>
                                <p class="text-left"> Paid : <strong> {{ $purchasePayment->total_pay }} </strong> BDT
                                </p>
                                <p class="text-left"> Due : <strong> {{ $purchasePayment->total_due }} </strong> BDT
                                </p>
                                <center>
                                    <p>Receipt No : {{ $purchasePayment->receipt_no }}</p>
                                </center>
                                <p class="text-left"> {{ $purchasePayment->created_at->format('d  F  Y') }}
                                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                    {{ $purchasePayment->created_at->format('h:i:s A') }}</p>
                            </div>
                            ...
                            <div class="row">
                                ...
                                <a href="{{ route('bank-branch.transfer.download', $purchasePayment->id) }}"
                                    class="hidden-print">
                                    <button type="submit"
                                        style="margin-right: 20px;border-color: #000000;background: white;"
